...
export default function ExploreMongolia() {
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      // Globe canvas fills the container width (minus nothing — contentRect is inner width)
      const w = Math.floor(Math.min(r.width, 720))
      // Height: keep a pleasant aspect on each breakpoint without dominating small screens
      const cap = isMobile ? 340 : isTablet ? 480 : 600
      const h = Math.floor(Math.min(w, cap))
      setSize({ w, h })
    })
  ...
  return (
    <section id="explore" className="py-14 sm:py-20 md:py-28 bg-bone relative overflow-hidden">
      <div className="pointer-events-none absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[400px] w-[400px] md:h-[800px] md:w-[800px] rounded-full bg-gradient-to-br from-sky-soft/40 via-transparent to-clay-soft/30 blur-3xl" />
      ...
      <div className="container-x relative">
        <div className="text-center max-w-3xl mx-auto mb-10 md:mb-14">
          ...
          <h2 className="h-display mt-4 text-3xl sm:text-4xl md:text-6xl text-ink">
            {(() => {
              const full = t({
